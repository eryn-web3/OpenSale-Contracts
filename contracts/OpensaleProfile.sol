...
// File: @satoshicrypto/matrixswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/OpensaleProfile.sol

pragma solidity ^0.6.0;

/** @title OpensaleProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract OpensaleProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public odcToken;
    ...
    uint256 public numberOdcToReactivate;
    uint256 public numberOdcToRegister;
    uint256 public numberOdcToUpdate;
    ...
    constructor(
        IBEP20 _odcToken,
        uint256 _numberOdcToReactivate,
        uint256 _numberOdcToRegister,
        uint256 _numberOdcToUpdate
    ) public {
        odcToken = _odcToken;
        numberOdcToReactivate = _numberOdcToReactivate;
        numberOdcToRegister = _numberOdcToRegister;
        numberOdcToUpdate = _numberOdcToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends ODC to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer ODC tokens to this contract
        odcToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberOdcToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer ODC token to this address
        odcToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberOdcToUpdate
    ...
    /**
     * @dev Claim ODC to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        odcToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of ODC to register
     * Callable only by owner admins.
     */
    function updateNumberOdc(
        uint256 newNumberOdcToReactivate,
        uint256 _newNumberOdcToRegister,
        uint256 _newNumberOdcToUpdate
    ) external onlyOwner {
        numberOdcToReactivate = newNumberOdcToReactivate;
        numberOdcToRegister = _newNumberOdcToRegister;
        numberOdcToUpdate = _newNumberOdcToUpdate;
    }
