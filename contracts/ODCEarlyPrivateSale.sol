...
contract ODCEarlyPrivateSale {
    ...
    uint256 public totalTokensToSell = 15000000 * 10**18;          // 15000000 ODC tokens for sell
    uint256 public odcPerBnb = 15000 * 10**18;             // 1 BNB = 15000 ODC
    ...
    mapping(address => uint256) public odcPerAddresses;
    ...
    // Function to buy ODC using BNB token
    function buyWithBNB(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = odcPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        odcPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        ODC.transfer(msg.sender, buyAmount);
    ...
    // function to set the total tokens to sell
    // only owner can call this function
    function setTokenPricePerBNB(uint256 _odcPerBnb) public {
        require(msg.sender == owner);
        require(_odcPerBnb > 0, "Invalid ODC price per BNB");
        odcPerBnb = _odcPerBnb;
    }
    ...
    function withdrawUnsoldTokens() public {
        ...
        ODC.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return ODC.balanceOf(address(this));
    }

    //function to calculate the quantity of ODC token based on the ODC price of bnbAmount
    function calculateODCAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 odcAmount = odcPerBnb.mul(bnbAmount).div(10**18);
        return odcAmount;
    }

    //function to calculate the quantity of bnb needed using its ODC price to buy `buyAmount` of ODC tokens.
    function calculateBNBAmount(uint256 odcAmount) public view returns (uint256) {
        require(odcPerBnb > 0, "ODC price per BNB should be greater than 0");
        uint256 bnbAmount = odcAmount.mul(10**18).div(odcPerBnb);
        return bnbAmount;
    }
}