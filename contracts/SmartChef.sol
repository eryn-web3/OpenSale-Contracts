...
contract SmartChef is Ownable {
    ...
    // Deposited amount ODC in SmartChef
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedOdc = depositedOdc.add(_amount).sub(depositFee).sub(transferTax);
            }else{
                uint256 transferTax = _amount.mul(odc.transferTaxRate()).div(10000);
                user.amount = user.amount.add(_amount).sub(transferTax);
                depositedOdc = depositedOdc.add(_amount).sub(transferTax);
            }
        }        
